...
class APIRequestor(object):
    ...
    def request_raw(self, meth, url, params={}):
        ...
        for attr, func in [['lang_version', platform.python_version], ['platform', platform.platform], ['uname', lambda: ' '.join(platform.uname())]]:
            try:
                val = func()
            except Exception, e:
                val = "!! %s" % e
    ...
    def requests_request(self, meth, abs_url, headers, params):
        ...
        try:
            try:
                result = requests.request(meth, abs_url, headers=headers, data=data, timeout=80, verify=os.path.join(os.path.dirname(__file__), 'data/ca-certificates.crt'))
            except TypeError, e:
    ...
    def handle_pycurl_error(self, e):
        if e[0] in [pycurl.E_COULDNT_CONNECT, pycurl.E_COULDNT_RESOLVE_HOST, pycurl.E_OPERATION_TIMEOUTED]:
...
class StripeObject(object):
    ...
    def __getattr__(self, k):
        ...
        if k in self._transient_values:
            raise AttributeError("%r object has no attribute %r. HINT: The %r attribute was set in the past, however. It was then wiped when refreshing the object with the result returned by Stripe's API, probably as a result of a save(). The attributes currently available on this object are: %s" % (type(self).__name__, k, k, ', '.join(self._values)))
        else:
